...
class StockAggregateParams(BaseModel):
    ...
    stock_ticker: str = Field(..., description="Case-sensitive ticker symbol (e.g., 'AAPL')")
    multiplier: int = Field(..., description="Size of the timespan multiplier")
    timespan: str = Field(..., description="Size of the time window")
    from_date: str = Field(..., description="Start date in YYYY-MM-DD format or millisecond timestamp")
    to_date: str = Field(..., description="End date in YYYY-MM-DD format or millisecond timestamp")
    adjusted: bool = Field(True, description="Whether results are adjusted for splits")
    sort: Optional[str] = Field(None, description="Sort results by timestamp ('asc' or 'desc')")
    limit: int = Field(5000, description="Maximum number of base aggregates (max 50000)")
...
def get_stock_aggregates(params: StockAggregateParams) -> Dict[str, Any]:
    ...
    query_params = {
        "adjusted": str(params.adjusted).lower(),
        "apiKey": API_KEY
    }
    ...
    while retry_count < Constants.MAX_RETRIES:
        try:
            ...
            print(f"Request failed (attempt {retry_count}/{Constants.MAX_RETRIES}): {str(e)}")
...
def print_stock_data(params: StockAggregateParams) -> str:
    ...
    for data in results:
        ...
        output.append("{:<12} {:<10.2f} {:<10.2f} {:<10.2f} {:<10.2f} {:<12.0f} {:<12.2f} {:<10} {:<12}".format(
            date_str, 
